...
- (void) authenticate:(CDVInvokedUrlCommand*)command {
  ...
  [self.commandDelegate runInBackground:^{
    ...
    if (keychainItemServiceName == nil) {
      ...
      keychainItemServiceName = [bundleID stringByAppendingString:@".AuthenticationPlugin"];
    }
    
    if (![self createKeyChainEntry]) {
      NSLog(@"Keychain trouble. Falling back to authenticateWithCustomPasswordFallback.");
      [self authenticateWithCustomPasswordFallback:command];
      return;
    }
...
- (void) authenticateWithCustomPasswordFallback:(CDVInvokedUrlCommand*)command {
  NSString *message = [command.arguments objectAtIndex:0];
  [self authenticateWithCustomPasswordFallback:command.callbackId withMessage:message andEnterPasswordLabel:nil];
}

- (void) authenticateWithCustomPasswordFallbackAndEnterPasswordLabel:(CDVInvokedUrlCommand*)command {
  NSString *message = [command.arguments objectAtIndex:0];
  NSString *enterPasswordLabel = [command.arguments objectAtIndex:1];
  [self authenticateWithCustomPasswordFallback:command.callbackId withMessage:message andEnterPasswordLabel:enterPasswordLabel];
}

- (void) authenticateWithCustomPasswordFallback:(NSString*)callbackId withMessage:(NSString*)message andEnterPasswordLabel:(NSString*)enterPasswordLabel {
